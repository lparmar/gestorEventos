          ...
          <div class="flex items-center mx-3">
            <div>
              <button type="button" class="flex text-sm rounded-full focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600" aria-expanded="false" data-dropdown-toggle="dropdown-user">
                <span class="sr-only">Open user menu</span>
                    ...
                    @else
                        <img class="w-8 h-8 rounded-full" src="{{Auth::user()->userProfile->getMedia('users_avatar')->first()->getUrl()}}" alt="Foto perfil">
                    @endif
              </button>
            </div>
            <div class="z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded shadow dark:bg-gray-700 dark:divide-gray-600" id="dropdown-user">
              ...
              <ul class="py-1" role="none">
                <li>
                  <a href="{{route('profile.edit')}}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-600 dark:hover:text-white" role="menuitem">Editar perfil</a>
                </li>
                <form method="POST" action="{{ route('logout')}}">
                    @csrf
                    <li class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-600 dark:hover:text-white">
...
<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700" aria-label="Sidebar">
   <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
      <ul class="space-y-2 font-medium">
         ...
         <li>
            <a href="{{route('users.show',Auth::user())}}" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-[#DEFFE5] dark:hover:bg-gray-700">
        ...
        @if (Auth::User()->roles()->first()->name == 'admin')
            ...
            <li>
                ...
                <span class="flex-1 ml-3 whitespace-nowrap">Profesores</span>
                </a>
            </li>
        @endif
      </ul>
