        ...
        @if ($body_activity !== null)
            <ul
                ...
                <li
                    ...
                    Actividades sobre: {{ $body_activity->name }} por confirmar
                </li>
            </ul>
            <div class="grid gap-6 m-3 md:grid-cols-2 pb-20">
                @foreach ($activities as $activity)
                    <a href="{{ route('activities-list-confirmAttendance.confirmAttendance', $activity) }}"
                        class="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
                        <img class="object-cover w-full rounded-t-lg h-96 md:h-auto md:w-48 md:rounded-none md:rounded-l-lg"
                            src="{{ $activity->getMedia('activities_avatar')->first()->getUrl() }}"
                            alt="{{ $activity->getMedia('activities_avatar')->first()->file_name }}">
